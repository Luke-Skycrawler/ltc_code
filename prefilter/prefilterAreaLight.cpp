...
int main(int argc, char* argv[])
{
    ...
    uint8_t* dataOut = new uint8_t[x * y * 3];

    // borders
    for (unsigned int level = 0; level < maxLevels + 1; ++level)
    {
        ...
        for (int i = 0; i < x; ++i)
        {
            ...
            dataOut[offset * 3] = uint8_t(r * 255.0); 
            dataOut[offset * 3 + 1] = uint8_t(g * 255.0); 
            dataOut[offset * 3 + 2] = uint8_t(b * 255.0); 
            offset ++;
        }
        else 
        for (int j = 0; j < y; ++j)
        for (int i = 0; i < x; ++i)
        {
            float r = imageInput(i, j, 0, 0);
            float g = imageInput(i, j, 0, 1);
            float b = imageInput(i, j, 0, 2);
        ...
        stbi_write_png(filenameOutput.str().c_str(), x, y, 3, dataOut, 0);
    }
    delete[] dataOut;
